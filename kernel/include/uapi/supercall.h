...
struct ksu_run_stage_cmd {
    __u32 stage_mask; /* Input: bit0=post-fs-data, bit1=services, bit2=boot-completed */
};

struct ksu_force_umount_cmd {
    __aligned_u64 arg; /* Input: pointer to mountpoint path string */
    __u32 flags;       /* Input: umount flags (0 = default, MNT_DETACH etc) */
    __s32 result;      /* Output: 0 on success, -errno on failure */
};

static const __u32 KSU_IOCTL_DISABLE_ESCAPE_TO_ROOT = _IO('K', 21);
static const __u32 KSU_IOCTL_RESET_BOOT_STATE = _IO('K', 22);
static const __u32 KSU_IOCTL_RUN_STAGE = _IOW('K', 23, struct ksu_run_stage_cmd);
static const __u32 KSU_IOCTL_FORCE_UMOUNT = _IOWR('K', 24, struct ksu_force_umount_cmd);
static const __u32 KSU_IOCTL_REAL_REBOOT_ONCE = _IO('K', 25);
